...
export default async function BlogCard({ post }: { post: SelectBlogs }) {
  ...
  return (
    <div className="rounded-md border border-border space-y-3 h-full w-full hover:bg-secondary bg-card text-card-foreground group">
      <Link href={`/blog/${post.slug}`} className="space-y-2 flex flex-col p-5 h-full">
        {
